...
這份文件是「比賽記錄與分析」這組新功能的規格入口，串接下面幾份文件。

開始讀之前，先看兩份**跨全專案、不屬於任何單一功能**的基礎文件：

- [`CONTEXT.md`](../CONTEXT.md)（repo 根目錄）——領域詞彙表。下面所有規格都照這份用詞。
- [`adr/`](./adr/)——架構決策紀錄。規格講「要做什麼」，ADR 講「為什麼不能改成另一種做法」。

| 文件                                                                   | 內容                                                              | 真正的規格來源                        |
| ---------------------------------------------------------------------- | ----------------------------------------------------------------- | ------------------------------------- |
| [product-vision.md](./product-vision.md)                               | 產品定位：給誰用、差異化、亮點、裝置形態（比下面幾份更上一層）    | 這份文件本身                          |
