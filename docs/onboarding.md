...
先照下面第 1、2 步把它跑起來，然後自己點一遍：建一場比賽 → 排先發 → 進計分表記幾球 → 開戰術板
畫兩筆。十分鐘就會知道使用者實際上在畫面上做什麼，比讀任何文件都快。

想知道「為什麼要做這些功能、背後哪兩股力在打架」，讀
[`requirements-pattern-language.md`](./requirements-pattern-language.md)。
