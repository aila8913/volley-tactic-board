...
\_Last updated: 2026-08-11 (aila) — **導覽資訊架構定案 → #372**（M6）：左欄改成「不需要先選比賽」
的入口，移除「計」、戰術脫離 `matchId`、「出」收回頁面、不做頁內 tab bar。情境依據落成
**`docs/usage-scenarios.md`**（本 PR 交付）。\_
...
- **有一份使用者情境盤點了：`docs/usage-scenarios.md`（08-11 交付，導覽討論的副產物）。**
  五大類使用者 × 十七個時機（A1–A5／B1–B6／C1–C3／D1／E1–E2），每個標裝置、身體約束、時間預算。
  大類沿用 `product-vision.md` 已定義的 TA，不另立一套。**它存在的理由是導覽爭論會原地打轉，
  因為每個人腦裡假設的情境不同而那個假設沒被講出來**——有編號之後可以問「你說的是 B3 還是 A3」。
  - **這輪討論的判準被推翻兩次，兩次都是漏了一個時機**：v1「全域 vs 單場」被 A5（平時純粹想戰術，
    根本還沒有那一場）推翻，v2「翻閱 vs 作業」被 E2（幫別隊當裁判）推翻，才收斂到 v3
    **「需不需要先指定一場比賽才有意義」**。過程記在 #372，因為推翻的理由比結論有用。
  - v1 形式其實是對的，它失敗是因為**把 `tactics.matchId` 綁死當成領域事實，而那只是實作缺陷**。
  - 盤點順帶照出兩個空白：**C1**（教練賽中借平板看一眼，30 秒）完全沒被設計過——現有分析頁全是為
    A4（從容坐著找洞察）做的；**B5**（記錯了要改）沒出現在任何既有文件裡，但球經在壓力下記錯是必然。
  - ⚠️ 討論中我一度說「overlay 已被 #154／#160 評估後否決」，**這是錯的、已更正**：ADR-0001 原話是
    「後來因為 Figma 新版面改成左欄工具頁」——是被換掉不是被否決，該 ADR 的「不要重新提議」三條
    全部關於資料流。真正的浮層實證在 `layout-spec.md` §2.2，但講的是**左欄 hover 展開**（沒叫它就
    自己冒出來、會遮住正要點的內容），不適用於「主動叫出、主動關掉」的 C1。
...
**導覽資訊架構要改，但還卡一個決策（#372，08-11 開，M6）。** 判準已定案（見上方 Current state
的情境盤點那條）：左欄只放「不需要先選比賽就有意義」的入口。**擋著動工的是「戰術要脫離
`tactics.matchId` 到什麼程度」**——最小改法是那一欄改可空（從某場存的照樣掛那場當標籤），
最徹底是換成自由標籤。沒有這一步，「戰」放進左欄只是換個地方灰掉。動 `lib/db/src/schema/`
的事要一次想清楚，且依協作規則要 heads-up @tangyi1025。另外兩件已經確定但還沒做：
「出」要從左欄收回戰術板頁（匯出 PNG 抓的是當前頁面 DOM，天生頁面相依，是個假的全域入口）、
第 4 點與 #214 的「頁內選擇比賽下拉」重疊，兩張票要對齊。**C1（教練賽中借平板 30 秒看一眼）
是獨立的新題**，現有分析頁都是為 A4 設計的，先定它要看什麼再決定是浮層還是頁面。
