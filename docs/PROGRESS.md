...
新開 #284**（工具軌圖示細節統整，留給下一輪）。自由球員全站視覺定案為「萊姆綠實色填滿＋深色
邊框」，跟前排球員同色（`#CCFF00`），戰術板珊瑚色虛線備位框拿掉。順手修掉一個嚴重但無關的
崩潰 bug（#276，已在 08-03 合併，見下方 08-01 前 aila 記錄同步）。\_
...
- **全站外殼材質（背景／右欄）已收斂成 `lib/appChromeStyles.ts`**（PR #253，**已合併**）：整頁背景
  ...
  分裂成兩代——複製的成本不在寫的當下，在改的時候。要調整全站氛圍改那一個檔案就好。**待調項目**（記在
  PR #253 留言，尚未動工）：右欄透明度 `/75` 的實際數值、`ListItemCard` 比例（要拿 Figma 重畫線稿，
  不再猜 px）、全站 16 種字級的收斂。
