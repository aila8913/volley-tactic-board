...
\_Last updated: 2026-07-26 (tang) — 計分頁球場置中偏右修正＋球員圈格式改用跟戰術板共用的
`PlayerMarker`（PR #210）；同一 session 在 #134 留言記錄 Track A（微 3D）延後、Track C（版面
呼吸空間）等功能介面穩定再做的決定，並開 #209 彙整目前發現的 UX 資訊密度問題（戰術板/計分頁
站位資訊重複顯示、右欄清單易觸發捲動）。前一批 2026-07-26 (tang) — #131 計分表整頁改版：
背景／球場材質改吃跟戰術板共用的 `courtTheme` 模組、版面改左右發展（PR #195）；合併時與 aila
的 #201（lineup/serving 死結修正）在同一段 JSX 衝突，已手動保留左右欄結構、套用修正後的判準。
前一批 2026-07-25 (aila) — #190–193 右欄三頁補齊（PR #201）＋修計分頁「有發球方沒先發」死結
（既有 bug，非本次引入；關聯 #64）。前一批 2026-07-25 (aila) — #127 補上
`tournamentBelongsToUser` 擁有權驗證。\_
...
- **深色語言已套完全站、手繪風全站退役，#131 於 07-23 關閉**（`docs/design-spec.md`，
  ...
  （#134 仍 open：Track A 微 3D 已拍板延後到很後面再討論；Track C 版面呼吸空間決定等
  功能介面先穩定下來再調整，目前介面本身還有哪些不夠流暢，彙整在新開的 #209。）
...
- **計分表整頁改版，球場材質與戰術板連動＋版面改左右發展（#131，PR #195，2026-07-26）**：抽出
  共用 `courtTheme` 模組，`Court.tsx`／`ScoreSheetCourt.tsx` 都吃同一份毛玻璃地板／邊緣光／漸層
  定義（改一處兩邊同步，不會再各自飄掉）；`ScoreSheet.tsx` 版面從上下擠一條改成左右發展（球場
  吃滿左邊剩餘寬度、比分＋操作按鈕收右邊一直欄，垂直置中、兩欄之間不加分隔線只留白）；背景加
  `.tb-beam` 斜向光影跟戰術板同一套材質語言。合併 main 時跟 aila 的 #201（lineup/serving 死結
  修正）在同一段「誰先發球」JSX 上衝突——結構不同（aila 那版還是舊的單欄版面），已手動把判準
  （改看 `lineup` 而非 `serving`）套進新的左右欄結構，語意上完整保留兩邊改動。
- **計分頁球場置中偏右修正＋球員圈格式與戰術板共用（PR #210，2026-07-26）**：#195 那次改版
  把球場欄寫成 `flex-1`，會先吃光整排剩餘寬度、自己在裡面置中，導致比分欄緊貼右側「場上站位」
  欄、留白全部集中在球場左邊，螢幕越寬越明顯往右偏（DOM 量測：1920px 視窗下左邊留白
  426px、右邊 0px）。改成球場欄跟內容一樣寬、外層整排 `justify-center`，兩邊留白現在對稱。
  同一顆 PR 也把球員圈畫法抽成共用元件 `components/PlayerMarker.tsx`（深色玻璃底＋狀態色
  邊框＋圈內背號／圈下姓名），`PlayerNode.tsx`（戰術板）跟 `ScoreSheetCourt.tsx`（計分表）
  現在吃同一顆，之後要調整球員圈樣式只要改一個檔案；計分表原本球員圈是「狀態色實心填滿＋
  單行文字（受一個沒有 UI 能切換的死開關 `circleLabel` 控制）」，統一成跟戰術板一樣的雙行格式。
...
- **PR #210**（無對應 issue，2026-07-26）— 計分頁球場置中偏右修正＋球員圈格式與戰術板共用：
  `ScoreSheet.tsx` 球場欄原本 `flex-1` 吃光剩餘寬度、自己在裡面置中，導致比分欄緊貼右側
  「場上站位」欄、留白全集中在球場左邊，螢幕越寬越明顯往右偏；改成球場欄跟內容一樣寬、外層
  整排 `justify-center`，兩邊留白對稱（DOM 量測驗證，非肉眼判斷）。同一顆 PR 抽出共用元件
  `components/PlayerMarker.tsx`，`PlayerNode.tsx`（戰術板）／`ScoreSheetCourt.tsx`（計分表）
  改吃同一份「深色玻璃底＋狀態色邊框＋圈內背號／圈下姓名」畫法，計分表原本被 `circleLabel`
  這個死開關控制的單行文字格式一併退役。**同一個 session 也在 #134 留言記錄視覺優先順序**：
  Track A（微 3D）延後、Track C（版面呼吸空間）等功能介面先穩定；連帶開 #209 彙整目前發現的
  UX 資訊密度問題（供 Track C 動工前參考）。
