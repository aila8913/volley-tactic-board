...
`環1 →（環2 ‖ 環3 ‖ 環4）→ 環5 → 環6`。** 環 1（#172）、環 2（#173）已關；環 4（#175）在 PR #196
待合併。**下一步是 #176（環 5 中央球場型＋模式 C）**——它依賴環 1、3、4，前置現在都到位了；接著
才是 #177（環 6 新增戰術流程）。#174 只剩 Stage B（統計格，blocked on M2），跨欄拖曳已由 PR #189
完成（issue body 的「未做」區塊還沒改掉，讀的時候留意）。#178（環 7 響應式）需先補線框稿。
...
state，否則會出現「左邊看第 2 局、右邊顯示第 3 局站位」）。
