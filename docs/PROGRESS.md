...
#394 唯讀紀錄表格 → **#395 球線分布＝M4 的 wow 點** → #396 表格可編輯 → #397 動作子分類。

⚠️ **#394 動工時會接到兩件 #393 刻意留下的事**：① 票面驗收的後半段「點該 rally 跳到影片
那一秒」是這張票的範圍——錨點（`rallies.videoId`／`videoTimestamp`）已經在寫了，缺的是讀它
的入口；② 「就地升級」的**程式碼**在 #396（表格可編輯）才需要，屆時 `writeLog.ts` 的
`WriteOp` 要長出 `patch`/`events` 這種 entry（目前只有 `create`）。規則本身在 ADR-0010
決定 3 與 #393 票面，不要重新推導一次。
