...
**#14** 的「一般球員疊到 L 那格」**08-10 起構造得出來了**——#327 補上第七格之後，排 L 先發終於有入口，
那個情境不再無法重現。實測（比賽列表右欄 + 計分頁右欄）沒有再看到疊圈：新模型下 L 根本不佔 lineup 的
格子，「兩個人在同一格」從型別上就不可表示。**#14 仍建議由 PO 自己走一次再關**，因為它原本回報的畫面
是戰術板球場（`Court.tsx`），不是右欄面板；
...
- **#231**（先發只留一種表示法，08-06 合併四個 PR、08-07 關閉）— 重構目標達成：`PerMatchRotationState`
  ...
  **判準值得記住：自動化驗證的是「每一處有沒有寫錯」，走一遍流程驗證的是「合起來還能不能用」，
  ...
  在右欄已驗不出來（新模型不可表示），**戰術板球場那條路仍待 PO 自己走一次**。
