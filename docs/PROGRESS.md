...
- **先發與自由球員的表示法只剩一份。** `PerMatchRotationState` 只存
  `lineup: LineupZones`（起始號位 → playerId，0~6 人皆合法）＋ `startingLiberoId`（誰是先發 L），
  六輪座標由 `deriveRotation(...)` 渲染時現算（`event-grammar-spec.md`
  那條**「能推導就不存」**的 store 版）。「可不可以開賽」獨立成 `isLineupFull`。
  - **賽前只回答「誰是先發 L」，不回答「他頂替誰」** →
    **[ADR-0013](adr/0013-pre-match-records-only-who-the-starting-libero-is.md)**（#425 推翻了
    #327 的一半）。四條「不要重新提議」裡最容易復發的是**「DB 那一欄恆為 null，一定是漏接了
    寫入路徑，補上吧」**——那條線是刻意不接的。
  - **L 的規則定案：從後排轉出去就下場、留在場外，直到手動再換上場**——系統不自動找下一個頂替對象。
    理由不是「猜得不準」，而是**這個 app 產出的是紀錄不是示意圖**。「L 不能跟著輪到前排」因此是
    一行推導，不需要清理邏輯。
  - 入口是輪轉表的**第七格**（3×2＋1）：存的是「誰是先發 L」不是「站哪格」，六個號位是輪轉序、
    L 不佔輪轉序（把 L 拖到那六格明確什麼都不做）。三個呼叫端＝計分頁右欄（真正的入口）／
    比賽列表右欄／戰術板右欄（唯讀，ADR-0001）。
  - 「L 現在頂替著誰」是**賽中**才成立的事實，住在 `useScoreSheet` 的 `record.liberoSubstitution`
    （球場上那顆 L 鈕寫入）。DB `lineups.libero_replaces_player_id` 記的是**開賽那一刻**的事實，
    前端一律凍成 null＝schema 註解列的合法狀態 (b)「指定了先發 L、但開局他在場外」。
...
- **有一份使用者情境盤點了：`docs/usage-scenarios.md`**（08-11，導覽討論的副產物）。
  五大類使用者 × 十九個時機（A1–A5／B1–B6／C1–C3／D1–D3／E1–E2），每個標裝置、身體約束、時間預算；
...
### 導覽資訊架構（#372，M6）——已實作，但三個版面答案是佔位的

左欄現在是固定四格 **比／數／戰／人**，全部永遠可點、目的地固定（判準＝「需不需要先指定
一場比賽才有意義」）。子清單機制整組退役，戰術板脫離 `matchId`：`/board` 是一級路由。

⚠️ **三處版面決定是我（Claude）代填的佔位答案，不是 tang 的定案**——原本這張票就是卡在
這三題上，PO 下週不在、要的是「有畫面可以被反應」。三個答案都寫成程式碼註解貼在現場：
選比賽下拉放在 board header 原本「vs 誰誰誰」的位置、「出」放同一條 header 右端（PNG 匯出
需要 `#court-wrapper`，只有這頁有）、位置調色盤與球員名單在同一個 flyout 並存。
**tang 推翻其中任何一個都不需要理由。**

- ⚠️ **`docs/layout-spec.md` §2 描述的是舊左欄**（比／計／數／戰＋子清單），已被這張票取代。
  該檔開頭已加註記，真正的重寫等 tang 的版面定案。
- ⚠️ **`captureFromScoreSheet`（`lib/courtSnapshot.ts`）現在沒有生產消費者**：計分頁那條
  「用這一局的凍結先發開一份戰術」的入口跟著子清單一起退役了。函式與測試都還在，
  是刻意留的（要重接入口時不用重寫），不是忘了刪。
...
- **「persist 永不能帶 match 資料」這條不變條件仍然只活在註解裡**（`useRotationTable.ts` 的
  `partialize` 旁邊）。#231 收尾時就提過該寫成 ADR，但一直沒寫；自由球員那半邊已經由
  ADR-0013 收掉，剩下的就是這一條。命中判準的理由沒變：看到 `partialize` 只留 `circleLabel`
  一個 key，很容易覺得「加個 `dataByMatch` 有什麼難的」就順手加了。
...
- **#425**（08-15）賽前不記「L 頂替誰」，第七格只回答「誰是先發 L」→
  **[ADR-0013](adr/0013-pre-match-records-only-who-the-starting-libero-is.md)**。
  兩件之後會被引用的事：① **拿掉入口之後 store 欄位就變成死值**，所以同一個 PR 一起刪
  （`liberoReplacesPlayerId` 沒有寫入者、恆為 null；留著只會讓下一個人以為賽前記得了頂替對象）。
  ② **DB 與 OpenAPI 刻意不動**——`lineups` 的註解本來就列了三種合法狀態，本張要的是 (b)
  「只有 `startingLiberoId`」，schema 早就允許，只是從例外變成常態。判準：**「前端不再寫」
  不等於「那個欄位到處都要刪」**，同名的三個東西（賽前規劃／賽中紀錄／開賽那一刻的事實）
  語意不同，對照表在 #425。
- **#328 ＋ #40**（08-15）戰術板中央「輪轉視圖」退役 →
  **[ADR-0012](adr/0012-tactics-board-center-draws-only-the-whiteboard.md)**（PR #423），
  收尾刪掉 `useRotationTable` 裡四支跟著失去消費者的寫入動作（PR #424）。
  **#40 隨之以 `not_planned` 關閉**——它描述的是「undo 不涵蓋輪轉視圖的拖曳」，畫面沒有了，
  缺口不是被修好而是沒有東西可以缺。這是 08-07 就在 #40 留言裡寫好的條件分支，不是臨時判斷。
  ⚠️ 右欄輪轉表的六格拖曳**仍然不進 undo**，那是另一個元件；真要做會先撞上「兩個 store、
  Ctrl+Z 語意變成看你在哪個視圖」那個沒解的設計題。
- **#401 ＋ #222**（08-15，PR #421）名單欄位按 Enter 直接 crash、新增球員兩條路徑行為不一致。
  同批補了 **#408 的第 2 案**（唯讀態的改名入口），但**#408 保持 open**：動工時發現票面的成因
  追不到（新增戰術根本不會停在唯讀檢視），而真缺口更大——**存下去的戰術 `name` 是空字串**，
  整條「新增 → 佈陣 → 編輯 → 儲存」沒有任何地方能輸入名字。第四個選項（存檔時給預設名字）
  已寫在該票留言，等 PO 拍板。
- **#376**（08-15，PR #422）文件：`usage-scenarios` 補 D2/D3，`product-vision` 的「刻意不做」
  拆成做／願景／捨棄三層。
- **#359**（08-13，PR #400）自由球員先發寫進 `lineups` 與逐局快照，歷史局與示範資料回看得到 L
  ——#327 交付當下那個「示範比賽看不到第七格」的洞補上了。
- **#372**（08-14）左欄改成「不需要先選比賽」的四格入口 ＋ 戰術板脫離 `matchId`。
  四件之後會被引用的事：① 後端 `GET /tactics` 多了 `?scope=global`——**query string 分不出
  「`?matchId=` 空值」和「根本沒帶 matchId」**，所以「只給我沒掛比賽的」必須另立旗標，不能
  靠 `matchId` 的有無表達（兩個都帶時 `matchId` 贏）。順帶把排序從 `createdAt` 升冪改成
  `updatedAt` 降冪——瀏覽清單給最舊的在最上面是個真 bug。② 空板存檔時用 `window.confirm`
  問「要不要掛在這場比賽底下」，**覆蓋既有戰術則永遠不問**（`UpdateTactic` 合約裡沒有
  `matchId` 這個欄位，改不了就別假裝問得到）。③ 位置調色盤拖出來的球員背號固定 `0`、
  圈裡改印角色代碼，判準寫在呼叫端 `PlayerNode.tsx` 而不是 `PlayerMarker` 裡面
  （「0 代表匿名」是資料模型的規則，畫圖的元件不該替呼叫端猜）。代價：真的有人穿 0 號
  也會顯示角色字母。④ 修掉四道 `if (!matchId) return;`（`Court.tsx` 的 Ctrl+Z effect／
  pointerDown／pointerMove、`PlayerNode.tsx` 的 pointerUp）——**那是「戰術板只存在於
  `/matches/:id/board`」年代的殘留**，undo 堆疊與 session 座標本來就跟哪一場無關。
  這條不在票面範圍裡，記在這裡是因為 #407 重寫繪圖工具時會經過同一批 handler，
  **而 #407 不會順手修掉它們**（那張票只提到刪掉三行 `setActiveTool("select")`）。
...
- **#329**（08-09）比賽編輯從彈窗搬進右欄就地編輯，`MatchFormDialog` 已刪。
  （當時掛著的相依「#222 的修法指向已刪除的元件」已隨 PR #421 一起解掉；#24 的 body 已改寫。）
