...
description: >
  ...
  commit → branch → push → PR → merge, with an explanation at every step (this is
  a learning project, teaching mode is on).
---

# Ship: commit → branch → push → PR → merge
...
- **If on `main`/`master`**: create and switch to a new branch.
  - Suggest a branch name based on the commit/feature (e.g. `feature/libero-fix`,
