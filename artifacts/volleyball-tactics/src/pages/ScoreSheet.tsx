          ...
          <div className="flex flex-1 flex-col min-h-0 border-r border-white/[0.08]">
            {!hasLineup ? (
              ...
              <div className="flex flex-1 flex-col items-center gap-2 px-4 py-3 overflow-y-auto">
                ...
                {/* 顯示「誰先發球」的判準同樣改看「有沒有凍結先發」（lineup）而非 serving
                  （見上面 canEditLineup 的說明）。正常流程兩者等價（開賽前 lineup 為 null →
                  顯示；開賽後 lineup 已凍結 → 顯示計分）；差別只在殘缺狀態：serving 已被寫過、
                  但 lineup 還是 null 時，這裡會「再問一次」發球方，讓教練重排滿 6 人後重選，
                  start() 這次就能把先發真正凍結＋補寫後端，那筆殘缺 set 自癒。 */}
                {!lineup || !currentSet ? (
