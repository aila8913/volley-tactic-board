...
//
// 這份快照現在的身分是「**該局的凍結歷史**」，不是計分表的平行先發資料。
//
// 站位的唯一真相是全站共用的 useRotationTable.dataByMatch[matchId]（#120 PO 定案）：比賽列表
// 決定它、計分表讀寫它、戰術板唯讀它。而歷史統計不能被事後編輯污染——所以教練選好先發方
// （開賽）那一刻，把當下的共用站位擷取成這一份快照存進 per-match 的計分 store，之後這一局
// 就只讀快照、不再跟著共用站位變動。換下一局時歸零，右欄自然退回顯示共用站位。
//
// 一句話：**共用現役 ＋ 開局凍結**。判斷式在 ScoreSheet.tsx 只有一行——
