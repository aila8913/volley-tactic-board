...
// 「新增戰術」全螢幕彈窗——issue #160 C2 開的、C3 改過擷取來源注入方式，issue #177 拆檔：
// 戰術板頁的「+ 新增戰術」改用 NewTacticDialog.tsx（只蓋中央球場的浮層，見該檔案開頭的
// 說明），但 NavRail 掛在「非戰術板頁」（比賽列表／計分頁／數據分析）時，畫面上根本沒有
// 中央球場欄可以蓋，所以那幾頁仍然需要這個原本的 Radix Dialog 全螢幕版本——按下「+ 新增
// 戰術」後選了起點，NavRail 會用 onStarted 把使用者導去戰術板頁，此時全螢幕彈窗才是正確
// 的體驗（使用者還沒進戰術板頁，沒有「只蓋中央欄」這件事可言）。
//
// 這個元件按「新增戰術」開這個彈窗，選一個起點：
...
interface NewTacticDialogModalProps {
  open: boolean;
...
export default function NewTacticDialogModal({
  open,
  onOpenChange,
  matchId,
  onStarted,
