...
export default function LeftPanel() {
  const { 
    ...
    rotations,
    resetCurrentRotation, clearMarkers
  } = useTactics();
    ...
    <div className="flex flex-col h-full bg-[#f8f8f8]">
      <div className="p-4 overflow-y-auto flex-1 space-y-5">
        ...
        {hasRotations && (
          <section>
            ...
            <div className="flex gap-2 mt-1">
              <button onClick={resetCurrentRotation} className="flex-1 wobbly-border bg-white px-2 py-1 text-xs font-bold hover:bg-gray-100">重置站位</button>
              <button onClick={clearMarkers} className="flex-1 wobbly-border bg-white px-2 py-1 text-xs font-bold hover:bg-red-100 text-red-600">清除畫筆</button>
            </div>
          </section>
        )}
        ...
        <section>
          ...
          {scenario !== 'base' && (
            <p className="text-[10px] text-gray-500 mt-1">此情境的站位獨立保存，切換情境不影響其他情境</p>
          )}
        </section>
        ...
        <section>
          ...
          <div className="flex gap-3">
            {(['name', 'role', 'zone'] as const).map(key => (
              <label key={key} className="flex items-center gap-1.5 cursor-pointer text-xs font-bold">
                <div className={`w-4 h-4 wobbly-border flex items-center justify-center ${labelToggles[key] ? 'bg-[#CCFF00]' : 'bg-white'}`}>
                  {labelToggles[key] && <div className="w-1.5 h-1.5 bg-[#111] rounded-full" />}
                </div>
