...
router.get("/analysis/people/:personId", async (req, res) => {
  const { personId } = GetPersonAnalysisParams.parse(req.params);
  ...
  if (!(await personBelongsToUser(personId, req.userId))) {
    res.status(404).json({ error: "Not found" });
    return;
  }
