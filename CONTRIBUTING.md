...
### 跨領域改動要對方確認（ship 流程的一部分）

每個人有自己主要負責的範圍（見下方分工表）。**動到對方範圍、或動到共用約定**的 PR，
merge 前要把對方加成 reviewer（`gh pr edit <編號> --add-reviewer <帳號>`），等對方
看過再合併，不要自己 squash merge 掉。「共用約定」指這些檔案：
...
只動自己範圍內的東西，照原本流程自己 merge 即可，不用每張 PR 都等對方。
...
改這些檔案本身就是「動到共用約定」，照上一節走：開 PR、把對方加成 reviewer、等確認
再 merge。這樣每一條約定的變更都有討論記錄，兩邊的 Claude 永遠讀到同一份。
