...
| label              | 說明                                                                                                  |
| ------------------ | ----------------------------------------------------------------------------------------------------- |
| `needs-plan`       | **動工時需要好好規劃**：方向可能早就定案了，但實作範圍大／牽動多處，動手前先進 Plan mode 把做法想清楚 |
| `epic`             | 傘狀彙整 issue：本身不直接動工，負責把一份盤點清單拆成可執行子票，子票拆完即退場                      |
| `help wanted`      | 想找夥伴一起討論或幫忙的 issue                                                                        |
| `good first issue` | 適合新加入的夥伴上手的入門任務——範圍小、規格清楚                                                      |
| `blocked`          | 卡住了，需要等其他 issue 先有進展                                                                     |

> **`needs-plan` 不代表「還沒想好」。** 這條寫得夠白是因為踩過：2026-08-07 盤點時，31 張 open
> 有 10 張帶 `needs-plan`，被讀成「三分之一還在討論階段」，差點被整批抽出去凍結成「待決策」區。
> 它是給**執行者**的施工指示（動手前先出計畫／先跟 PO 對一次做法），不是 issue 的定案狀態。
> 要判斷一張票到底定案沒，讀 issue 留言與產出物，不要看標籤。**當下狀態一律看
> [Project #4](https://github.com/users/aila8913/projects/4) 的 Status 欄**——標籤管分類、
> milestone 管時程、Project 管狀態，三個軸各司其職，不要讓標籤兼差當狀態。
